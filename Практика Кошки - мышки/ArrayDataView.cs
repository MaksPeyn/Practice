...
namespace Практика_Кошки___мышки
{
	...
	public class ArrayDataView: IBindingList
	{
		...
		{
			if (array.Rank != 2)
				throw new ArgumentException(@"Supports only two dimentional arrays",nameof(array));
		...
		public ArrayDataView(Array array, object[] colnames) : this(array)
		{
			if(colnames.Length != array.GetLength(1))
				throw new ArgumentException(@"column names must correspond to array columns.",nameof(colnames));
...
//		#endregion // Events
		...
		public System.ComponentModel.ListSortDirection SortDirection => new System.ComponentModel.ListSortDirection ();
